class Node:
    ...
    def insert(self, payload, parent_stack):
        if self.payload is None:
            # empty tree
            self.payload = payload
            return self
            ...
            if self.left is None:
                ...
                return self.left.insert(payload, parent_stack + [(self, True)])
        elif payload == self.payload:
            ...
            if self.right is None:
                ...
                return self.right.insert(payload, parent_stack + [(self, False)])

    def __splay(self, parent_stack):
        if parent_stack == []:
            return
            # finished, already root
